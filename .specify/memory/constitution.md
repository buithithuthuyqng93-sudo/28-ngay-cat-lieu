...
bảng `Order` chỉ được thêm khi tích hợp cổng thanh toán PayOS thật (xem
`specs/002-payos-payment-integration`) — không thêm bảng/service tương tự cho nhu cầu
chưa thực sự phát sinh. Ba dòng lặp lại còn tốt hơn một abstraction sớm và sai.
...
`markAsPaid` cho tới khi thay bằng PayOS thật ở `specs/002-payos-payment-integration`.)
